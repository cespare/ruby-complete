...
module CompletionTool
  ...
  def self.init
    registration_file = CompletionTool.registration_file
    abort "File already exists: #{registration_file}." if File.exists? registration_file
    header =<<-EOM
# WARNING -- AUTOGENERATED FILE
# Modifying this file by hand is not recommended -- use `completion.rb register`
# or `completion.rb unregister` instead.
EOM
    File.open(registration_file, "w").write(header)
    register("completion.rb")
  end
  ...
  def self.register(command)
    ...
    if File.file?(completion_script)
      ...
      `source #{CompletionTool.registration_file}`
    else
